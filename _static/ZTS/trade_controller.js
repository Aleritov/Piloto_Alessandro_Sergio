...
if (parseInt(localStorage.cur_day) === length - 2) {
    ...
    // update views
    update_portfolio();
    $_('trade_price').innerHTML = y.toFixed(2);
    $_('trade_news').innerHTML = news[parseInt(localStorage.cur_day)];

    // finalizar
    clearInterval(interval_func);
    liveSend(get_trade_report('End', y, 0));
    alert('La etapa ha terminado, continue presionando "Siguiente" en la parte inferior.');
    disable_buttons();
    localStorage.clear();
    $('.otree-btn-next').show();
...
function set_buy_sell_amounts() {
    ...
    $_('trade_btn_sell_s').innerHTML = "Vender " + values[0];
    $_('trade_btn_sell_s').value = values[0];
    $_('trade_btn_buy_s').innerHTML = "Comprar " + values[0];
    $_('trade_btn_buy_s').value = values[0];
    $_('trade_btn_sell_m').innerHTML = "Vender " + values[1];
    $_('trade_btn_sell_m').value = values[1];
    $_('trade_btn_buy_m').innerHTML = "Comprar " + values[1];
    $_('trade_btn_buy_m').value = values[1];
    $_('trade_btn_sell_l').innerHTML = "Vender " + values[2];
    $_('trade_btn_sell_l').value = values[2];
    $_('trade_btn_buy_l').innerHTML = "Comprar " + values[2];
...
function buy_shares(amount) {
    ...
    if(parseInt(localStorage.cur_day) >= 0) {
        ...
        if(cur_total <= parseFloat(localStorage.cash)) {
            ...
            liveSend(get_trade_report('Comprar', cur_price, amount));
            ...
            update_portfolio();
            // send report to server
            liveSend(get_trade_report('Comprar', cur_price, available_amount));
...
function sell_shares(amount) {
    ...
    if(parseInt(localStorage.cur_day) >= 0) {
        ...
        if(amount <= cur_shares) {
            ...
            update_portfolio();
            // send report to server
            liveSend(get_trade_report('Vender', cur_price, -amount));
            toastr.remove(); toastr.success('Success!');
        }
            ...
            update_portfolio();
            // send report to server
            liveSend(get_trade_report('Vender', cur_price, -available_amount));
